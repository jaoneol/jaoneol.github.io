...
![WSL Intro](/assets/img/2025-01-27-Docker1_1.png)
...
![WSL Installation](/assets/img/2025-01-27-Docker1_2.png)
![WSL Installation](/assets/img/2025-01-27-Docker1_3.png)
_WSL Installation_
...
- Ideally, it would be best to simply move the VHDX file to the D drive, but since Ubuntu OS was installed while enabling WSL, it was installed as a Windows App and may have other dependencies. Therefore, after moving the files to a different path, additional configurations need to be updated. <ins>Since manually checking and handling all these tasks can be complex, there is a project on GitHub that provides a script for this process.</ins>    
	`wsl --shutdown`    
...
![Change the default path of WSL](/assets/img/2025-01-27-Docker1_7.png)
...
- Let's check the wsl's configure(`/etc/wsl.conf`) to use 'systemd'
	...
	In Ubuntu 24.04, systemd is enabled by default, but it is not in older versions. This was included for verification purposes. If `ps -p 1 -o comm=` does not return `systemd`, the `systemctl` command will not work. Keep this in mind.    		
...
Ubuntu-24.04에는 기본적으로 systemd가 활성화 되어 있으나, 과거 버전은 안되어 있다. 확인차 넣어둔 내용이다. 만약 ps -p 1 -o comm= 에서 systemd가 안되어 있으면 systemctl 명령어가 동작하지 않는다. 유의해라.
-->
...
필요 패키지 설치 : apt-get install -y ubuntu-restricted-extras net-tools git strongswan iperf ssh libssl-dev libncurses5 libncurses5-dev bin86 ftpd wireshark speedometer mpv vim cmake build-essential WSL 메모리 제한 (.wslconfig 파일 내용) : [wsl] memory=4GB
-->
...
To remove the installed Ubuntu from WSL, simply enter the command below for easy removal. However, be extremely careful, as this action is irreversible with a single command. The virtual disk file `ext4.vhdx` will be permanently deleted, so make sure to back it up before proceeding. (I will explain Import & Export in the next post.)

`wsl --unregister Ubuntu`        

<!--
WSL에서 설치된  Ubuntu를 제거하려면, 아래 명령만 입력하면 쉽게 제거가 됩니다. 매우 주의할 점은 한번의 명령으로 복구 자체가 불가능하다는 점입니다. 가상 디스크 파일인 ext4.vhdx 이 바로 삭제되므로 반드시 백업해주시고 진행바랍니다.(다음 글에서 Import & Export 설명할께요.)
-->
